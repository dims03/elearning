...
class ChaptersRelationManager extends RelationManager
{
    ...
    protected static ?string $title = 'Chapter & Subjects';
        ...
        return $table
            ...
            ->recordActions([
                ...
                Actions\Action::make('manage_lessons')
                    ...
                    ->modalWidth('5xl')
                    ->fillForm(fn ($record) => [
                        'lessons' => $record->lessons->map(fn ($lesson) => [
                            'id' => $lesson->id,
                            'title' => $lesson->title,
                            'type' => $lesson->type,
                    ...
                    ->form([
                        Forms\Components\Repeater::make('lessons')
                            ...
                            ->cloneable()
                            ->collapsible()
                            ->itemLabel(fn (array $state) => (($state['type'] ?? null) ? strtoupper($state['type']) . ' - ' : '') . ($state['title'] ?? 'Materi baru'))
                            ->columnSpanFull(),
                    ])
